...
public abstract class AbstractTestCase {
	...
	private void testByCase(UnitCaseContext context, UnitTestCase stockTestCase) throws Exception {
		logger.info(stockTestCase.getName() + " case 准备开始测试");
		//清理环境
		clearEnvironment(context, stockTestCase);
		logger.info(stockTestCase.getName() + " case 开始前的清理数据完成");
		try {
			//测试启动
			start(context, stockTestCase);
			sleep(100);
			logger.info(stockTestCase.getName() + " case 测试用例启动");
			//准备数据
			prepare(context, stockTestCase);
			sleep(100);
			logger.info(stockTestCase.getName() + " case 数据准备完成");
			//操作执行
			doAction(context, stockTestCase);
			sleep(100);
			logger.info(stockTestCase.getName() + " case 动作执行完成");
			//对比数据
			diffData(context, stockTestCase);
			logger.info(stockTestCase.getName() + " case 对比工作完成");
		}
		finally {
			if (clearAfterTestCaseEnd) {
				//清理环境
				clearEnvironment(context, stockTestCase);
				logger.info(stockTestCase.getName() + " case 结束后的清理数据完成");
			}
			//关闭环境
			try {
				...
				logger.error(" case 测试用例结束异常", ex);
			}

		}
		logger.info(stockTestCase.getName() + " case 成功结束测试");
	}
